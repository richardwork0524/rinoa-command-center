...
export function TaskAddBar({ projects, selectedProject, onSubmit, position = "top" }: TaskAddBarProps) {
  ...
  return (
    <div className={`${positionClass} bg-surface border-border px-4 py-3`}>
      <form onSubmit={handleSubmit} className="flex items-center gap-2">
        {projects && projects.length > 0 && (
          <select
            value={projectKey}
            onChange={(e) => setProjectKey(e.target.value)}
            className="h-9 px-2 rounded-md bg-surface-2 border border-border-light text-[13px] text-text-2 focus:outline-none focus:border-primary max-w-[100px] truncate"
          >
            {projects.map((p) => (
        ...
        <select
          ...
          className="h-9 px-2 rounded-md bg-surface-2 border border-border-light text-[13px] text-text-2 focus:outline-none focus:border-primary"
        >
          {BUCKETS.map((b) => (
        ...
        <input
          ...
          className="flex-1 h-9 px-3 rounded-md bg-surface-2 border border-border-light text-[14px] text-text-1 placeholder:text-text-3 focus:outline-none focus:border-primary"
        />
        <button
          type="submit"
          disabled={submitting}
          className="h-9 px-4 rounded-md bg-primary text-white text-[13px] font-semibold hover:bg-primary-dim disabled:opacity-40 transition-colors"
        >
          {submitting ? "..." : "+"}
        </button>
      </form>
